...
int rpc_qcsapi_set_key_passphrase(const char *wpa_psk)
{
	...
	if (ret < 0) {
		...
		ret = qcsapi_wifi_set_pre_shared_key(WIFINAME, 0, wpa_psk);
		if (ret < 0)
			dbG("Qcsapi qcsapi_wifi_set_pre_shared_key error, return: %d\n", ret);
...
ej_wl_status_5g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	qcsapi_unsigned_int association_count, tx_phy_rate, rx_phy_rate, time_associated;
	qcsapi_mac_addr sta_address;
	int hr, min, sec;
	...
	if (ret < 0) {
		...
		for (i = 0; i < association_count; ++i) {
			...
			ret = qcsapi_wifi_get_associated_device_mac_addr(WIFINAME, i, (uint8_t *) &sta_address);
			if (ret < 0)
				dbG("Qcsapi qcsapi_wifi_get_associated_device_mac_addr error, return: %d\n", ret);

			ret= qcsapi_wifi_get_rssi_in_dbm_per_association(WIFINAME, i, &rssi);
			if (ret < 0)
				dbG("qcsapi_wifi_get_rssi_in_dbm_per_association error, return: %d\n", ret);

			ret = qcsapi_wifi_get_tx_phy_rate_per_association(WIFINAME, i, &tx_phy_rate);
			if (ret < 0)
				dbG("qcsapi_wifi_get_tx_phy_rate_per_association error, return: %d\n", ret);

			ret = qcsapi_wifi_get_rx_phy_rate_per_association(WIFINAME, i, &rx_phy_rate);
			if (ret < 0)
				dbG("qcsapi_wifi_get_rx_phy_rate_per_association error, return: %d\n", ret);

			ret = qcsapi_wifi_get_time_associated_per_association(WIFINAME, i, &time_associated);
			if (ret < 0)
				dbG("qcsapi_wifi_get_time_associated_per_association error, return: %d\n", ret);

			hr = time_associated / 3600;
			min = (time_associated % 3600) / 60;
			sec = time_associated - hr * 3600 - min * 60;

			ret += websWrite(wp, "%s ", wl_ether_etoa((struct ether_addr *) &sta_address));

			ret += websWrite(wp, "%-11s%-11s", "Yes", !nvram_match("wl1_auth_mode_x", "open") ? "Yes" : "No");

			ret += websWrite(wp, "%4ddBm ", rssi);

			ret += websWrite(wp, "%6dM ", tx_phy_rate);

			ret += websWrite(wp, "%6dM ", rx_phy_rate);
