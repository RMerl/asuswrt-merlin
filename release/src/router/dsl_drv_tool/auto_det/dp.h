...
static struct client_config_t client_config = {
        ...
        interface: "eth2.2.1", //2008.08 magic
        pidfile: NULL,
...
int send_dhcp_discover(struct client_config_t *client_config);//Yau
...
#define DEFAULT_IF "eth2.2"  //2008.08 magic changed DEFAULT_IF "eth0" 
...
typedef struct PPPoAoverPPPoEPacketStruct {
    struct ethhdr ethHdr;       /* Ethernet header */
    unsigned int ver:4;         /* PPPoE Version (must be 1) */
    unsigned int type:4;        /* PPPoE Type (must be 1) */
    unsigned int code:8;        /* PPPoE code */
    unsigned int session:16;    /* PPPoE session */
    unsigned int length:16;     /* Payload length */
    unsigned char payload[100]; /* A bit of room to spare */
} PPPoAoverPPPoEPacket;
...
//2010.08.31 Yau add
typedef struct DetectPvcStruct {
	short vpi;
	short vci;
	char  encap;
//	char  mode;
...
typedef struct DetectInfoStruct {
	PPPoEConnection conn;
