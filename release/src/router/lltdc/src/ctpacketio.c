...
/*
void *
fmt_qltlv(uint8_t *buf, uint8_t type, uint16_t offset) 
{
    IF_TRACED(TRC_PACKET)
        printf("fmt_qltlv: type=%x offset=%d\n", type, offset);
    END_TRACE

    topo_qltlv_header_t *qhdr = (topo_qltlv_header_t*)(buf + sizeof(topo_ether_header_t) + sizeof(topo_base_header_t) + 1);

    qhdr->qh_type = type;
    qhdr->qh_rsvd1 = 0;
    qhdr->qh_offset = offset;
...
//                   buf,     src-addr,      dst-addr,            service-type,        g_opcode,         seq,       B'cast
	...
	memcpy((void*)(dhdr+1),&qryMAC,sizeof(etheraddr_t)); //Yau
...
void
packetio_send_query_large_tlv(uint16_t thisSeqNum, etheraddr_t* pDst)
{
    size_t                      nbytes;
    topo_qltlv_header_t 	*qhdr;

    //             			  buf,    src-addr,  dst-addr, service-type,    g_opcode,    	     seq,       B'cast
    qhdr = (topo_qltlv_header_t*)fmt_base(g_txbuf,&g_hwaddr, pDst,ToS_TopologyDiscovery,Opcode_QueryLargeTlv,thisSeqNum,FALSE);

    qhdr->qh_type = 0x0e; //icon image
    qhdr->qh_rsvd1 = 0;
    qhdr->qh_offset = g_rcvd_icon_len;
    swapbytes16(qhdr->qh_offset);

    nbytes = sizeof(topo_ether_header_t) + sizeof(topo_base_header_t) + sizeof(topo_qltlv_header_t);

    tx_sendpacket(nbytes, thisSeqNum);

    IF_TRACED(TRC_PACKET)
        dbgprintf("\n==== tx_queryLargetlv: seqnum = %d, dst= " ETHERADDR_FMT " Len = %d ===\n\n",
                   thisSeqNum, ETHERADDR_PRINT(pDst), g_rcvd_icon_len);
    END_TRACE
}
