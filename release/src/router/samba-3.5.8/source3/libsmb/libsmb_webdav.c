...
fprintf(stderr, "enter smbc_cli_opendir2..with fname=[%s]\n", fname);
	...
	if (server[0] == (char)0) {
		...
		/*
		  ...
		  */
		fprintf(stderr, "start name_resolve_bcast\n");
		ip_list = NULL;
		...
		fprintf(stderr, "leave name_resolve_bcast, max_lmb_count=[%d], count=[%d]\n\n", max_lmb_count, count);
		for (i = 0; i < count && i < max_lmb_count; i++) {
			...
			fprintf(stderr, "-->Found master browser %d of %d: %s\n", i+1, MAX(count, max_lmb_count), addr);
			
			cli = get_ipc_connect_master_ip(talloc_tos(), &ip_list[i], &u_info, &wg_ptr);

			fprintf(stderr, "@@@leave get_ipc_connect_master_ip\n");
	...
	fprintf(stderr, "leave smbc_cli_opendir2..\n\n");
