...
handle_pwd(struct vsf_session* p_sess)
{
	...
	str_replace_text(&temp_cwd_buf_mangle_str, "\"", "\"\"");
	/* Enclose pathname in quotes */
	str_alloc_text(&s_pwd_res_str, "\"");
	tmp_str = local2remote(str_getbuf(&temp_cwd_buf_mangle_str));
	if (tmp_str == NULL) {
		str_append_str(&s_pwd_res_str, &temp_cwd_buf_mangle_str);
	} else {
...
handle_mkd(struct vsf_session* p_sess)
{
  ...
  if(p_sess->layer != SHARE_LAYER || !p_sess->is_anonymous){
    ...
    if(new_folder == NULL){
      vsf_cmdio_write(p_sess, FTP_FILEFAIL,
                      "1. Create directory operation failed.");
      return;
    }
  ...
  {
    vsf_cmdio_write(p_sess, FTP_FILEFAIL,
                    "2. Create directory operation failed.");
    return;
  }
  vsf_log_do_log(p_sess, 1);
  {
...
handle_rmd(struct vsf_session* p_sess)
{
  ...
  if(p_sess->layer != SHARE_LAYER || !p_sess->is_anonymous){
    ...
    if(folder == NULL){
      vsf_cmdio_write(p_sess, FTP_FILEFAIL,
                      "Remove directory operation failed (001).");
      return;
    }
