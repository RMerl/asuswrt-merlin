...
static void
webstr_init(struct ip6t_entry_match *m)
{
	return;
}
...
static void
webstr_save(const void *ip, const struct ip6t_entry_match *match)
{
	printf("--webstr ");
	print_string(((struct ipt_webstr_info *)match->data)->string,
...
static
struct xtables_match webstr_match6 = {
    .family = NFPROTO_IPV6,
    .name = "webstr",
...
void _init(void)
{
	xtables_register_match(&webstr_match6);
}
