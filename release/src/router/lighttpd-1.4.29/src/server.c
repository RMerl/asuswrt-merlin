...
#define DBG_ENABLE_SERVER 0
#define DBE	DBG_ENABLE_SERVER
			...
			if ((srv->cur_fds + srv->want_fds > srv->max_fds * 9 / 10) || /* out of fds */
				...
				for (i = 0; i < srv->srv_sockets.used; i++) {
					...
					if (graceful_shutdown) {
						 ...
						 *  */
						Cdbg(1, "graceful_shutdown lighttpd process");
...
Cdbg(DBE, "GOT event, fd_ndx=[%d], fd=[%d], revents=[%04X]\n", fd_ndx, fd, revents);
		...
		for (ndx = 0; ndx < srv->joblist->used; ndx++) {
			...
			handler_t r;
			//Cdbg(1,"call connection_state_machine con=%p", con);
