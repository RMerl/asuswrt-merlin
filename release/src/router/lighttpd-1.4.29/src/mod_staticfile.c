...
//0
	...
	for (s = con->request.http_range, error = 0;
		...
		if (!error) {
			...
			Cdbg(DBE,"phy = %s, url = %s", con->physical.path, con->url.path->ptr);
			if(con->mode == DIRECT){
			   Cdbg(DBE,"direct append");
				chunkqueue_append_file(con->write_queue, con->physical.path, start, end - start + 1);
			}
			else
			{
			   Cdbg(DBE,"ntlm basic append");
...
Cdbg(DBE, "set to 403\n\n\n\n\n");
	...
	if(con->mode == SMB_BASIC || con->mode == SMB_NTLM){
	   Cdbg(DBE,"SMB_BASIC NTLM");
		http_chunk_append_smb_file(srv, con, con->url.path, 0, sce->st.st_size);
	}
	else
	{
	   Cdbg(DBE,"DIRECT");
