...
lighttpd_monitor_SOURCES=log.c buffer.c lighttpd-monitor.c

lighttpd_arpping_SOURCES=log.c buffer.c arpping_thread.c nvram_control.c
#lighttpd_arpping_CFLAGS = $(AM_CFLAGS) $(LIBSMBCLIENT_CFLAGS)
...
common_src=buffer.c log.c \
      ...
      splaytree.c status_counter.c asus_util.c \
	    mod_smbdav_ntlm.c mod_smbdav_basic.c smb_auth.c nvram_control.c
...
lib_LTLIBRARIES += mod_smbdav.la 
mod_smbdav_la_SOURCES = mod_smbdav.c
mod_smbdav_la_CFLAGS = $(AM_CFLAGS) $(XML_CFLAGS) $(SQLITE_CFLAGS)
mod_smbdav_la_LDFLAGS = -module -export-dynamic -avoid-version -no-undefined
mod_smbdav_la_LIBADD = $(common_libadd) $(XML_LIBS) $(SQLITE_LIBS) $(UUID_LIBS)
...
lib_LTLIBRARIES += mod_aidisk_access.la
mod_aidisk_access_la_SOURCES = mod_aidisk_access.c
mod_aidisk_access_la_LDFLAGS = -module -export-dynamic -avoid-version -no-undefined
mod_aidisk_access_la_LIBADD = $(common_libadd)

lib_LTLIBRARIES += mod_create_captcha_image.la
mod_create_captcha_image_la_SOURCES = mod_create_captcha_image.c
mod_create_captcha_image_la_LDFLAGS = -module -export-dynamic -avoid-version -no-undefined
mod_create_captcha_image_la_LIBADD = $(common_libadd)

lib_LTLIBRARIES += mod_sharelink.la
mod_sharelink_la_SOURCES = mod_sharelink.c
mod_sharelink_la_LDFLAGS = -module -export-dynamic -avoid-version -no-undefined
mod_sharelink_la_LIBADD = $(common_libadd)
...
hdr = server.h buffer.h network.h log.h keyvalue.h \
      ...
      mod_magnet_cache.h \
      version.h asus_util.h smb_auth.h nvram_control.h
