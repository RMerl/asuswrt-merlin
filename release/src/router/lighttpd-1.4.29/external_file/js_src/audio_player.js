...
function openAudioPlayer(loc){
	...
	get_mp3_url(audio_list, true, function(audio_playlist){
		g_audioPlayer.addPlaylist(audio_playlist);
		g_audioPlayer.showHideAudioPlayerWindow(true);
	});
	
	/*
	get_mp3_url(loc, true, function(audio_playlist){
		g_audioPlayer.addPlaylist(audio_playlist);
		g_audioPlayer.showHideAudioPlayerWindow(true);
	});
	*/
...
function openAudioPlayerByPlayList(playlist){
	...
	g_audioPlayer.addPlaylist(playlist);	
	g_audioPlayer.showHideAudioPlayerWindow(true);
}
		...
		var timer = setInterval(function(){
			...
			if(on_query==false){
				...
				if(current_query_index<0||current_query_index>this_audio_list.length-1){
					clearInterval(timer);
					complete_handler(audio_playlist);
					return;
				}
...
function initAudioPlayer(){
	...
	AudioPlayer.prototype = {
		...
		showHideAudioPlayerWindow: function(bshow){
			var self = this;
			if(bshow){
