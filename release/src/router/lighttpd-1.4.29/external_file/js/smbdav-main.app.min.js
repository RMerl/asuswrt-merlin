...
1;if(generate_sharelink==1){var n=window.location.host;n.indexOf(":")!=-1&&(n=n.substring(0,n.indexOf(":")));var n="http://"+n+":"+g_storage.get("http_port")+"/",k=setInterval(function(){if(f==!1)if(g<0||g>e.length-1)clearInterval(k),initJPlayer(),showHideLoadStatus(!1),$.mobile.changePage("#audio_dialog",{transition:"pop",role:"dialog"});else{var a=e[g],b=a.substring(a.lastIndexOf("/")+1,a.length),a=a.substring(0,a.lastIndexOf("/"));f=!0;client.GSL(a,a,b,0,0,function(a,c,d){if(a==200)a=parseXml(d),
a=$(a).find("sharelink").text(),a=n+a,f=!1,c=[],c.name=mydecodeURI(b),c.mp3=a,g_audio_playlist.push(c),g++})}},100)}else{for(b=0;b<e.length;b++)d=e[b],a=d.substring(d.lastIndexOf("/")+1,d.length),d.substring(0,d.lastIndexOf("/")),c=[],c.name=mydecodeURI(a),c.mp3=d,g_audio_playlist.push(c);initJPlayer();showHideLoadStatus(!1);$.mobile.changePage("#audio_dialog",{transition:"pop",role:"dialog"})}}}}
...
function createOpenUrlUI(a){a==void 0&&(a="/");var c=mydecodeURI(a),b="",e="",d="",g=$("#urlregion-url").width();if(c!="/")for(var c=c.split("/"),a=a.split("/"),f=0;f<c.length;f++)if(!(c[f]==""||g_enable_aidisk==1&&c[f]==g_aidisk_name)){e+="/"+a[f];d+=" / <a id='url_path' uhref='"+e+"'>"+c[f]+"</a>";if(String(d).width($("p#openurl").css("font"))>g){b+=" / ...";break}b+=c[1]==g_aidisk_name&&g_enable_aidisk==1?" / <a id='url_path' uhref='/"+g_aidisk_name+e+"'>"+c[f]+"</a>":" / <a id='url_path' uhref='"+
e+"'>"+c[f]+"</a>";d=b}$("p#openurl").empty();$("p#openurl").append(b);$("a#url_path").click(function(){doPROPFIND($(this).attr("uhref"))})}
function doPROPFIND(a,c,b){if(client!=null){showHideLoadStatus(!0);try{client.PROPFIND(a,b,function(b,e,f){if(b){if(b==207){cancelSelectMode();closeUploadPanel();g_folder_array=[];g_file_array=[];if(window.DOMParser)b=new DOMParser,e=b.parseFromString(f,"text/xml");else if(e=new ActiveXObject("Microsoft.XMLDOM"),e.async="false",e.loadXML(f),!e.documentElement){alert("Fail to load xml!");showHideLoadStatus(!1);return}var n,k,o,h,p,G=e.documentElement.childNodes,f=e.documentElement.getAttribute("qtype");
e.documentElement.getAttribute("readonly");b=e.documentElement.getAttribute("ruser");e.documentElement.getAttribute("computername");e=e.documentElement.getAttribute("isusb");for(n=0;n<G.length;n++){var l="",t="",u="",q="",v="",w="",x="",y="",z="",A="",B="",C="",D="",j,E="0",r=G[n].childNodes;for(k=0;k<r.length;k++)if(r[k].nodeType==1&&r[k].nodeName=="D:propstat"){var F=r[k].childNodes;for(o=0;o<F.length;o++)if(F[o].nodeName=="D:prop"){var i=F[o].childNodes;for(h=0;h<i.length;h++)if(!(i[h].childNodes.length<=
...
"D:getip")z=String(i[h].childNodes[0].nodeValue);else if(i[h].nodeName=="D:gettype")B=String(i[h].childNodes[0].nodeValue);else if(i[h].nodeName=="D:getattr"){var s=i[h].childNodes;for(p=0;p<s.length;p++)if(s[p].nodeName=="D:readonly")C=s[p].childNodes[0].nodeValue;else if(s[p].nodeName=="D:hidden")D=s[p].childNodes[0].nodeValue}else i[h].nodeName=="D:getroutersync"&&(E=String(i[h].childNodes[0].nodeValue))}}else r[k].nodeType==1&&r[k].nodeName=="D:href"&&(l=String(r[k].childNodes[0].nodeValue),o=
"",l.match(/^http/)&&(o=window.location.protocol+"//"+window.location.host),h=o+addPathSlash(a),l!=h?(l=l.replace(o,""),q=l.substring(l.lastIndexOf("/")+1,l.length),q!=""?(j=q,v=q=mydecodeURI(q)):l=""):l="");l!=""&&(t=="httpd/unix-directory"?g_folder_array.push({contenttype:t,href:l,name:q,uname:j,shortname:v,online:w,time:x,size:y,ip:z,mac:A,uid:u,type:B,freadonly:C,fhidden:D,routersyncfolder:E}):g_file_array.push({contenttype:t,href:l,name:q,uname:j,shortname:v,online:w,time:x,size:y,ip:z,mac:A,
uid:u,type:B,freadonly:C,fhidden:D,routersyncfolder:E}))}g_folder_array.sort(sortByName);g_file_array.sort(sortByName);for(j=0;j<g_folder_array.length;j++)g_folder_array[j].type=="usbdisk"&&arraymove(g_folder_array,j,0);j=addPathSlash(a);j!="/"&&(j=j.substring(0,j.length-1),j=j.substring(0,j.lastIndexOf("/")),j==""&&(j="/"));j=="/"+g_storage.get("modalname")&&(j="/");createListView(f,j,g_folder_array,g_file_array);f==1&&e==0?$("#btn-changeuser").show():$("#btn-changeuser").hide();f==0?$("#btn-select").show():
...
function openSelItem(a){var c=a.attr("uhref"),b=a.attr("qtype"),e=a.attr("isdir"),d=a.attr("isusb"),g=a.attr("uhref"),f=a.attr("title");g_storage.set("openuid",a.attr("uid"));g_storage.set("opentype",d);if(b==2&&a.attr("online")==0)confirm(m.getString("wol_msg"))==!0&&(c=a.attr("mac"),client.WOL("/",c,function(a){a==200?alert(m.getString("wol_ok_msg")):alert(m.getString("wol_fail_msg"))}));else if(e=="1")doPROPFIND(c,null,null);else if(a=getFileExt(c),g_storage.get("webdav_mode"),a=="mp3")openAudioPlayer(c);
...
b=f=d.substring(0,d.lastIndexOf("/"));e+=encodeURI(c);e+=";"});if(e=="")alert("Please select files first.");else{var d=g_storage.get("webdav_mode"),g=g_storage.get("ddns_host_name"),f=g==""?window.location.host:g;f.indexOf(":")!=-1&&(f=f.substring(0,f.indexOf(":")));isPrivateIP(f);f=d==0?"http://"+f+":"+g_storage.get("http_port"):"https://"+f;client.GSL(b,b,e,86400,1,function(b,e,d){if(b==200)if(b=parseXml(d),b=a(b).find("sharelink").text(),b=="")alert("Fail to parse xml!");else{b=b.split(";");e=
b.length;for(d=0;d<e;d++)c+=f+"/"+b[d],d!=e-1&&(c+=";");openShareWindow(c)}})}});a("a#btn-delete").live("click",function(){var c=function(a){a&&a[0]!=2?alert(m.getString(a)+" : "+decodeURI(g_selected_files[0])):(g_selected_files.splice(0,1),g_selected_files.length<=0?(a=addPathSlash(g_storage.get("openurl")),doPROPFIND(a)):client.DELETE(g_selected_files[0],c))};g_selected_files=null;g_selected_files=[];a("li#list_item.selected").each(function(){a(this).attr("uhref");g_selected_files.push(a(this).attr("uhref"))});
