...
function doRENAME(a,b){var c=0,e=addPathSlash(g_storage.get("openurl")),f=e+b;$(".wcb").each(function(){b==myencodeURI($(this).attr("data-name"))&&(c=1,$(this).attr("isdir")=="1"?alert(m.getString("folder_already_exist_msg")):alert(m.getString("file_already_exist_msg")))});c!=1&&g_webdav_client.LOCK(a,"",function(b,d,c){if(b!="201")alert(m.getString("msg_already_lock"));else{var h=getLockToken(c);g_webdav_client.MOVE(a,f,function(b){b[0]==2?(doPROPFIND(e),closeJqmWindow(0)):alert(m.getString(b));
...
b+"?mobile=1"}else if(a=="favorite")addtoFavorite();else if(a=="config"){var b=g_storage.get("http_enable"),c=g_storage.get("misc_http_enable"),d=g_storage.get("misc_http_port"),g=g_storage.get("misc_https_port"),h=window.location.host,k="http",a=d;c==0&&!isPrivateIP()?alert(m.getString("msg_no_config")):(b=="0"?(k="http",a=d):b=="1"&&(k="https",a=g),isPrivateIP()?b=k+"://"+h.split(":")[0]:(b=k+"://"+h.split(":")[0],a!=""&&(b+=":"+a)),window.location=b)}else a=="help"?(a=$("div#modalWindow"),b=getPageSize(),
