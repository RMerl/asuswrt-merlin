...
int fill_auth_on_req(unsigned char *buf);

int fill_debug_req(unsigned char *buf);

int fill_eap_key_req(unsigned char *buf, unsigned char *key, int key_len);

/* 00 02 config requests */

int fill_string_info_req(unsigned char *buf);

int fill_get_nspid_req(unsigned char *buf);
//int fill_diode_control_cmd(unsigned char *buf, int turn_on);

int fill_mac_req(unsigned char *buf);

/* 03 12(14) data from NV requests */
...
int fill_find_network_req(unsigned char *buf, struct wimax_dev_status *dev);

int fill_connect_req(unsigned char *buf, struct wimax_dev_status *dev);
...
/*02 00*/
//int fill_eap_server_rx_req(unsigned char *buf, unsigned char *param, int param_len);
int fill_eap_server_rx_req(unsigned char *buf, unsigned char *param, int param_len);
/*03 0c*/
...
/*00 06*/
int fill_rf_on_req(unsigned char *buf);
/*00 04*/
int fill_rf_off_req(unsigned char *buf);
