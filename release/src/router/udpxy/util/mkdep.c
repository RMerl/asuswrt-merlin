...
void do_depend(const char * filename, const char * command)
{
	...
	if (hasdep) {
		if (*command) {
			...
			/*
			 * Don't try to touch through symlinks
			 * [Not the fastest way to do it, but we do it only once.]
			 * >>>mv '?' '?.sym'&&cp '?.sym' '?'&&rm -f '?'.sym\n\0<<<
			 *     4    3      11        7         10         7 ==> 42
			 */
			if (S_ISLNK(st.st_mode)) {
				int   len = strlen(command);
				char *cmd = malloc((5*len) + 42);
				if (!cmd) {
					perror ("out of memory");
					goto DONE;
				}
				sprintf (cmd,	"mv '%s' '%s.sym'&&"
						"cp '%s.sym' '%s'&&"
						"rm -f '%s.sym'\n",
					command, command,
					command, command,
