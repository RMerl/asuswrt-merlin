...
#include "includes.h"
#include "ntdomain.h"
#include "librpc/gen_ndr/srv_dnsserver.h"

static bool api_dnsserver_foo(struct pipes_struct *p)
{
	...
	call = &ndr_table_dnsserver.calls[NDR_DNSSERVER_FOO];

	r = talloc(talloc_tos(), struct dnsserver_foo);
	if (r == NULL) {
		return false;
	}
	...
	if (DEBUGLEVEL >= 10) {
		NDR_PRINT_FUNCTION_DEBUG(dnsserver_foo, NDR_IN, r);
	}
	...
	if (DEBUGLEVEL >= 10) {
		NDR_PRINT_FUNCTION_DEBUG(dnsserver_foo, NDR_OUT | NDR_SET_VALUES, r);
	}
...
{
	{"DNSSERVER_FOO", NDR_DNSSERVER_FOO, api_dnsserver_foo},
};
