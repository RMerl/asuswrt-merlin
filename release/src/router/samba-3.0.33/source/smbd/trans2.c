	...
	*got_exact_match = False;
	...
	while (!found) {
		...
		if(got_match) {
			 ...
			 * with a pending modtime. */
			fsp = file_find_di_first(sbuf.st_dev, sbuf.st_ino);
			if (fsp && !null_timespec(fsp->pending_modtime)) {
				mdate_ts = fsp->pending_modtime;
			} else {
				...
				struct timespec ts[2])
{
	...
	round_timespec(conn->ts_res, &ts[0]);
	round_timespec(conn->ts_res, &ts[1]);
	...
	ts_stat[0] = get_atimespec(psbuf);
	ts_stat[1] = get_mtimespec(psbuf);
	round_timespec(conn->ts_res, &ts_stat[0]);
	round_timespec(conn->ts_res, &ts_stat[1]);

	if (timespec_compare(&ts[0], &ts_stat[0]) ||
			timespec_compare(&ts[1], &ts_stat[1])) {
