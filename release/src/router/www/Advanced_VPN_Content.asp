...
function addRow_Group(upper, flag){
	...
	}else if (flag == 'openvpn'){
		var table_id = "openvpn_clientlist_table";
	...
	if(validForm(flag)){
		...
		if(item_num >=2){
			for(i=0; i<rule_num; i++){	
				if(username_obj.value.toLowerCase() == $(table_id).rows[i].cells[0].innerHTML.toLowerCase()){
					alert("<#JS_duplicate#>");
					username_obj.focus();
					username_obj.select();
					return false;
		...
		for(k=0; k<$('openvpn_clientlist_table').rows.length; k++){
			for(j=0; j<$('openvpn_clientlist_table').rows[k].cells.length-1; j++){
				if(j == 0)	
					...
					vpn_server_clientlist_value += $('openvpn_clientlist_table').rows[k].cells[0].innerHTML;
					vpn_server_clientlist_value += ">";
					vpn_server_clientlist_value += $('openvpn_clientlist_table').rows[k].cells[1].innerHTML;
				}
			}
		}
...
function showopenvpn_server_clientlist(){
	...
	code +='<tr><td width="40%"><% nvram_get("http_username"); %></td><td width="40%"></td><td width="20%"></td></tr>';
				...
				for(var j = 0; j < vpn_server_clientlist_col.length; j++){
						if(j == 0){
								if(vpn_server_clientlist_col[0].length >32){
										...
										code +='<td width="40%" title="'+overlib_str0[i]+'">'+ vpn_server_clientlist_col[0] +'</td>';
								}else
										code +='<td width="40%">'+ vpn_server_clientlist_col[0] +'</td>';
						}
						else if(j ==1){
								if(vpn_server_clientlist_col[1].length >32){
										...
										code +='<td width="40%" title="'+overlib_str1[i]+'">'+ vpn_server_clientlist_col[1] +'</td>';
								}else
										code +='<td width="40%">'+ vpn_server_clientlist_col[1] +'</td>';
						} 
				}
				code +='<td width="20%">';
				code +='<input class="remove_btn" onclick="del_Row(this, \'openvpn\');" value=""/></td></tr>';
		}
	}
  code +='</table>';
	$("vpn_server_clientlist_Block").innerHTML = code;
}
...
$j.ajax({
    		...
    		success: function(){
      			if(vpnd_state == '0'){
      					setTimeout("update_vpn_server_state();", 1000);
      			}else{	// OpenVPN server ready , vpn_server1_state==1
									...
									<div id="OpenVPN_setting" style="display:none;">
										<div class="formfontdesc">
											...
											<ol>
												...
												<li><a href="http://support.asus.com/search.aspx?SLanguage=en&keyword=OpenVPN%20mac" target="_blank" style="text-decoration:underline;">Mac OS</a>
												<li><a href="http://support.asus.com/search.aspx?SLanguage=en&keyword=OpenVPN%20iphone" target="_blank" style="text-decoration:underline;">iPhone/iPad</a>
												<li><a href="http://support.asus.com/search.aspx?SLanguage=en&keyword=OpenVPN%20android" target="_blank" style="text-decoration:underline;">Android</a>
											<ol>
										</div>										
										...
										<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable_table" style="margin-top:8px;">
											...
											<tr>
												...
												<td width="20%">
													<div><input type="button" class="add_btn" onClick="addRow_Group(16, 'openvpn');" value=""></div>
												</td>
											</tr>	 			  
										</table>        
														
										<div id="vpn_server_clientlist_Block"></div>	
										<!-- manually assigned the DHCP List end-->		
									</div>
