...
function show_leases() {
	...
	if ("<% nvram_get("dhcp_enable_x"); %>" == "0") {
		...
		for (i = 0; i < leasearray.length-1; ++i) {
			...
			code += '<td><span class="ClientName" onclick="oui_query(\'' + line[1] +'\');;overlib_str_tmp=\''+ overlib_str +'\';return overlib(\''+ overlib_str +'\');" onmouseout="nd();" style="cursor:pointer; text-decoration:underline;">'+ line[1] +'</span></td>'; 
			code += '<td>' + line[2] + '</td>';
			code += '<td>' + line[3] + '</td>';
			code += '</tr>';
		}
