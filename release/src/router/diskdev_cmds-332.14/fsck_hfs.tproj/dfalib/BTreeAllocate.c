...
OSStatus	AllocateNode (BTreeControlBlockPtr		btreePtr, UInt32	*nodeNum)
{
	...
	//////////////////////// Find Word with Free Bit ////////////////////////////
		...
		size  >>= 1;						// convert to number of words
						//ее assumes mapRecords contain an integral number of words
...
OSStatus	FreeNode (BTreeControlBlockPtr		btreePtr, UInt32	nodeNum)
{
	...
	++btreePtr->freeNodes;
	btreePtr->flags |= kBTHeaderDirty;					//ее how about a macro for this
						 ...
						 UInt32					newTotalNodes )
{
	...
	//ее update for proper 64 bit arithmetic!!
	...
	newTotalNodes = filePtr->fcbLogicalSize / nodeSize;		//ее hack!
	//ее do we wish to perform any verification of newTotalNodes at this point?

	btreePtr->totalNodes = newTotalNodes;		//ее do we need to update freeNodes here too?
						...
						UInt16					 *mapSize )
{
	...
	if (nodePtr->buffer != nil)		// if iterator is valid...
	{
		...
		{
			err = fsBTInvalidHeaderErr;				//ее or fsBTBadNodeType
			goto ErrorExit;
		}
