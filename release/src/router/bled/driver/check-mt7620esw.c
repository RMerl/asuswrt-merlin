...
/* copy from rt_mmap.h, mt7620.h, and raether.c */
...
/**
 * Get TX/RX bytes from MT7620 embedded switch.
 * @port:
 * @rx:
 * @tx:
 */
...
{
	*rx = _ESW_REG(REG_ESW_PORT_RGOCN_P0 + port * 0x100);
	*tx = _ESW_REG(REG_ESW_PORT_TGOCN_P0 + port * 0x100);
}
