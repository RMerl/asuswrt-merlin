...
#else
#define USB_CDC_SEND_ENCAPSULATED_COMMAND 0x0
#define USB_CDC_GET_ENCAPSULATED_RESPONSE 0x1
#endif
#endif
#include <net/arp.h>
...
#define NETDEV_HAVE_DEV (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 20))
#if NETDEV_HAVE_DEV
...
#endif
#endif
#define HAVE_RESET_RESUME (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 22))
...
#define DRIVER_VERSION			"1.12-Option"
...
#endif
static void tiocmget_intr_callback(CALLBACK_ARGS);
/*****************************************************************************/
/* Helping functions                                                         */
/*****************************************************************************/
...
#define safe_usb_unlink_urb usb_unlink_urb
#define safe_usb_kill_urb   usb_kill_urb
#else
static void safe_usb_unlink_urb(struct urb *urb)
{
	if (!urb)
		return;
...
/* Sysfs attribute */
static ssize_t hso_sysfs_show_porttype(struct class_device *class_dev, char *buf)
{
	struct hso_device *hso_dev = class_dev->dev->driver_data;
	char *port_name;
...
static CLASS_DEVICE_ATTR(hsotype, S_IRUGO, hso_sysfs_show_porttype, NULL);
#endif
...
#elif (LINUX_VERSION_CODE == KERNEL_VERSION(2, 6, 19))
	dev_err(serial->parent->class_dev->dev, "hso_urb_to_index failed\n");
#endif
	return -1;
}
...
static void hso_net_tx_timeout(struct net_device *net)
{
	...
	if (odev->mux_bulk_tx_urb
		...
		safe_usb_unlink_urb(odev->mux_bulk_tx_urb);
...
#endif
	if (result2)
		released = hso_kref_put(&serial->parent->ref, hso_serial_ref_free);
...
static void tiocmget_intr_callback(CALLBACK_ARGS)
{
	...
	if (serial_state_notification->bmRequestType != BM_REQUEST_TYPE ||
		...
		DUMP((unsigned char *)serial_state_notification,
		     sizeof(struct hso_serial_state_notification));
	} else {
...
static int hso_stop_net_device(struct hso_device *hso_dev)
{
	...
	for (i = 0; i < MUX_BULK_RX_BUF_COUNT; i++) {
		if (hso_net->mux_bulk_rx_urb_pool[i])
			safe_usb_kill_urb(hso_net->mux_bulk_rx_urb_pool[i]);

	}
	if (hso_net->mux_bulk_tx_urb)
		safe_usb_kill_urb(hso_net->mux_bulk_tx_urb);
...
static int hso_stop_serial_device(struct hso_device *hso_dev)
{
	...
	for (i = 0; i < serial->num_rx_urbs; i++) {
		if (serial->rx_urb[i]) {
				safe_usb_kill_urb(serial->rx_urb[i]);
	...
	if (tiocmget) {
		wake_up_interruptible(&tiocmget->waitq);
		safe_usb_kill_urb(tiocmget->urb);
	}
	...
	if (serial->tx_urb)
		safe_usb_kill_urb(serial->tx_urb);
...
#elif (LINUX_VERSION_CODE == KERNEL_VERSION(2, 6, 19))
    if (serial->parent->class_dev)
        class_device_remove_file(serial->parent->class_dev, &class_device_attr_hsotype);
#endif
...
#elif (LINUX_VERSION_CODE == KERNEL_VERSION(2, 6, 19))
    serial->parent->class_dev = tty_register_device(tty_drv, minor,
                                              &serial->parent->
                                              interface->dev);
    if (serial->parent->class_dev) {
        serial->parent->class_dev->dev->driver_data = serial->parent;
        if (class_device_create_file(serial->parent->class_dev, &class_device_attr_hsotype))
            printk(KERN_WARNING"Could not create ttyHS? in sysfs");
    }
#else
...
static struct hso_device *hso_create_net_device(struct usb_interface *interface, u32 port_spec)
{
	int result, i;
	struct net_device *net;
	...
	net = alloc_netdev(sizeof(struct hso_net), "hso", hso_net_init);
	if (!net) {
...
static void hso_free_interface(struct usb_interface *interface)
{
	...
	for (i = 0; i < HSO_SERIAL_TTY_MINORS; i++) {
		if (serial_table[i]
			...
			released = hso_kref_put(&serial_table[i]->ref,
					hso_serial_ref_free);
			mutex_unlock(hso_mutex);
		}
	}
...
#endif
#if defined(CONFIG_USB_SUSPEND) && defined(CONFIG_HSO_AUTOPM)
