...
/* define GPIOs*/
#define FAN_GPIO        (1 << 14)       // GPIO 14
#define PWR_LED_GPIO    (1 << 12)       // GPIO 12
#define RST_BTN_GPIO    (1 << 9)        // GPIO 9
#define HAVE_FAN_GPIO   (1 << 6)        // GPIO 6
#define EZ_BTN_GPIO     (1 << 4)        // GPIO 4
#endif
...
bool rmode_set(void) /* reset mode */
{
        ...
        si_detach(sih);
        return gpioin & EZ_BTN_GPIO ? FALSE : TRUE;
}
...
extern void GPIO_INIT(void)
{
        ...
        si_gpiocontrol(sih, FAN_GPIO, 0, GPIO_DRV_PRIORITY);
        si_gpioouten(sih, FAN_GPIO, 0, GPIO_DRV_PRIORITY);
        si_gpiocontrol(sih, HAVE_FAN_GPIO, 0, GPIO_DRV_PRIORITY);
        si_gpioouten(sih, HAVE_FAN_GPIO, 0, GPIO_DRV_PRIORITY);
}
...
extern void FANON(void)
{
        sih = si_kattach(SI_OSH);
        ASSERT(sih);
        si_gpioouten(sih, FAN_GPIO, FAN_GPIO, GPIO_DRV_PRIORITY);
        si_gpioout(sih, FAN_GPIO, 0, GPIO_DRV_PRIORITY);
        si_gpioouten(sih, HAVE_FAN_GPIO, HAVE_FAN_GPIO, GPIO_DRV_PRIORITY);
        si_gpioout(sih, HAVE_FAN_GPIO, 0, GPIO_DRV_PRIORITY);
}
unsigned char DETECT(void)
{
        unsigned char d = 0;
	...
	strcpy(os_name, "nflash0.os");
		...
		if (check_trx(trx_name)) {
			...
			xprintf("go load\n");	// tmp test
                        ...
                        /* Load the image */
                        sprintf(buf, "load -raw -addr=0x%x -max=0x%x :", (unsigned int)ptr, (unsigned int)TRX_MAX_LEN);
