/*  *********************************************************************
    ...
    *  
    *  bcm6345_board.c   utility functions for bcm6345 board
    *  
...
static int parseMemConfig(char *memSizeStr)
{
    ...
    if (memSize >= MEMORY_8MB_1_CHIP && memSize <= MEMORY_64MB_2_CHIP)
        return 0;
    else
        return 1;
}
...
static PARAMETER_SETTING gBoardParam[] =
{
    ...
    {"Psi size in KB (default:15)  :", PSI_SIZE_PROMPT, "", "", parsePsiSize},
    {"Number of MAC Addresses(1-32):", MAC_CT_PROMPT, "", "", parseMacAddrCount},
    {"Base MAC Address             :", MAC_ADDR_PROMPT, "", "", parseMacAddr},
    {"Memory size:   Range 0-3\n\
8MB  (1 chip)  ------- 0\n\
16MB (1 chip)  ------- 1\n\
32MB (1 chip)  ------- 2\n\
64MB (2 chips) ------- 3     :", MEM_CONFIG_PROMPT, "", "", parseMemConfig},
    {NULL}
};
...
void getBoardParam(void)
{
    ...
    if (nvramData.ulNumMacAddrs == 0xffffffff)
        nvramData.ulNumMacAddrs = NVRAM_MAC_COUNT_DEFAULT_6345;
    ...
    if (!(memType >= MEMORY_8MB_1_CHIP && memType <= MEMORY_64MB_2_CHIP))
        memType = MEMORY_8MB_1_CHIP;
...
void displayBoardParam(void)
{
    ...
    switch (memType)
    {
    case MEMORY_8MB_1_CHIP:
        strcpy(megaByte, "8MB");
        break;
    case MEMORY_16MB_1_CHIP:
        strcpy(megaByte, "16MB");
        break;
    case MEMORY_32MB_1_CHIP:
        strcpy(megaByte, "32MB");
        break;
    case MEMORY_64MB_2_CHIP:
