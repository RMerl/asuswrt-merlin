...
static struct usb_device_id InterfaceUsbtable[] = {
	...
	{ USB_DEVICE(BCM_VENDOR_ID_ASUS, BCM_PRODUCT_ID_25E) },
	{ USB_DEVICE(BCM_VENDOR_ID_ASUS, BCM_PRODUCT_ID_35E) },
	{ USB_DEVICE(BCM_VENDOR_ID_ZTE, BCM_PRODUCT_ID_AX226) },
	{ USB_DEVICE(BCM_VENDOR_ID_ZTE, BCM_PRODUCT_ID_AX320) },
    {}
};
MODULE_DEVICE_TABLE(usb, InterfaceUsbtable);

int DeviceInsertedCount = 0;
...
static int usbbcm_ioctl(struct inode *inode, struct file *file,
...
//	BCM_DEBUG_PRINT(Adapter,DBG_TYPE_INITEXIT, DRV_ENTRY, DBG_LVL_ALL, "Usbbcm probe!!");
    ...
    if(DeviceInsertedCount > BCM_MAX_DEVICE_SUPPORTED)
    {
          printk("<<<<<More than 1 devices are not supported>>>>>\n");
          return -EOPNOTSUPP;
    }
	...
	if(psAdapter->chip_id > T3)
	{
		...
		{
			BCM_DEBUG_PRINT(psAdapter,DBG_TYPE_INITEXIT, DRV_ENTRY, DBG_LVL_ALL, "Register Write Failed: %x\n", DISABLE_USB_ZERO_LEN_INT);
		}
	}
